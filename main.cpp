...
void display() {
	...
	// bottom-right
	mat4 view2 = translate(identity_mat(), vec3(width / 2, height / 2, 30.0));
	mat4 persp_proj2 = perspective(30.0, ((float)width / 2) / ((float)height / 2), 0.1, 100.0);
	mat4 model2 = rotate_y_deg(identity_mat4(), 30.0);
	...
	// top-left
	vec3 cam_pos = { 0, height / 2, 0.0 };
	vec3 obj_pos = { width / 4, (height / 4) * 3, 0.0f };
	vec3 up = { 0.0, 0.0, 1.0 };
	mat4 view3LookAt = lookAt(cam_pos, obj_pos, up);

	glUniformMatrix4fv(proj_mat_location, 1, GL_FALSE, view3LookAt.m);
	...
	// top-right
	//orthographic POV
	mat4 ortho_proj = Ortho(width / 2, width, height / 2, height, 0.1, 100.0);

	glUnifromMatrix4f(proj_mat_location, 1, GL_FALSE, ortho_proj.m);
